...
The Git repository contains configure templates for Mac OS X and several Linux distributions with the right paths for set (e.g. @configure.template.macosx@). These still assume that you have installed the correct packages (e.g. @python-dev@ and the JDK).

After you have run @configure@, you can build Mesos with @make@.

_Note: the @configure@ script currently prints a warning at the end about "unrecognized options: --with-java-home, --with-python headers, ...". This is actually from one of the nested @configure@ scripts that we call, so it does not mean that your options weren't used._
