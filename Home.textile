Mesos is a cluster resource manager that efficiently shares and isolates resources among multiple distributed applications, or _frameworks_. It currently supports "Hadoop MapReduce":http://hadoop.apache.org, "MPI":http://www.mcs.anl.gov/research/projects/mpich2/, and "Spark":http://github.com/mesos/spark (a new MapReduce-like framework from the Mesos team that supports low-latency interactive and iterative jobs). Mesos can also serve as a platform upon which to build new distributed applications (i.e. frameworks), with the advantage that new applications will be able to share resources with existing applications (such as Hadoop, MPI, etc) and can therefore easily be adopted by these communities.
