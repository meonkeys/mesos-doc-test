...
Mesos is an open source <i><b>cluster operating system</b></i>. Unlike traditional operating systems (e.g. Linux or Windows) that support applications on a single computer, Mesos manages tens to thousands of computers, providing a platform on which to build <i>distributed applications</i> (which we also call <i>frameworks</i>). For example, "Hadoop MapReduce":http://hadoop.apache.org, "MPI":http://www.mcs.anl.gov/research/projects/mpich2/, "Hypertable":http://hypertable.org, and "Spark":http://github.com/mesos/spark (a new MapReduce-like framework from the Mesos team that supports low-latency interactive and iterative jobs), and many other applications, run on Mesos!

h3. You can use Mesos to...
* Run Hadoop, MPI, Hypertable, Spark and other distributed applications on a shared pool of machines.
* Run multiple instances of Hadoop on the same cluster -- say separate instances for production and experimental jobs -- or even multiple versions of Hadoop (to test out a new version).
* Build new cluster applications without having to reinvent low-level facilities for farming out tasks to different nodes, monitoring them, etc, and have your apps coexist with existing ones.
...
* [[Configuring Mesos|Configuration]] -- A guide to the various conf. settings that can be passed to Mesos dameons
