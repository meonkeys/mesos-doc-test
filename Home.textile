...
* Fault-tolerant replicated master node (using "Apache ZooKeeper":http://hadoop.apache.org/zookeeper/).
* Support for isolating frameworks on each machine using Linux Containers.
...
Mesos uses "Google Logging":http://code.google.com/p/google-glog and writes logs to @MESOS_HOME/logs@ by default, where @MESOS_HOME@ is the location where Mesos is installed. The log directory can be [[configured|Configuration]] using the @log_dir@ parameter.
...
* [[Mesos architecture]] -- an explanation of Mesos internals.
