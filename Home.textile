...
Mesos is a cluster resource manager that efficiently shares and isolates resources among multiple distributed applications, or _frameworks_. Mesos is an operating system, but not like a traditional OS (e.g. Linux or Windows). Mesos is an OS at the cluster level; in other words, it is a platform on which to build distributed applications (i.e. frameworks), with the advantage that new applications will be able to share resources with existing applications (such as Hadoop, MPI, etc) and can therefore easily be adopted by these communities. Mesos currently supports "Hadoop MapReduce":http://hadoop.apache.org, "MPI":http://www.mcs.anl.gov/research/projects/mpich2/, and "Spark":http://github.com/mesos/spark (a new MapReduce-like framework from the Mesos team that supports low-latency interactive and iterative jobs).
