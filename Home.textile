...
_*Please note that Mesos is still in alpha. This release is only intended for early testing.*_
...
* @--with-zookeeper=DIR@ or @--with-included-zookeeper@: Enable master fault-tolerance using an existing ZooKeeper installation or the version of ZooKeeper bundled with Mesos. For more help, see [[using ZooKeeper]].

The Git repository contains configure scripts for Mac OS X and several Linux distributions with the right paths for each platform. These assume that you have installed the correct packages (e.g. @python-dev@ and the JDK).
...
# Run the C++ test framework (a sample application that just executes five tasks on the cluster) using @bin/examples/cpp-test-framework [URL of master]@. It should successfully finish after running five tasks.

Once you have run this test, you will probably want to set up a larger cluster. You can use the included [[deploy scripts]] to launch a cluster on a set of nodes, or Mesos's [[EC2 scripts]] to launch a cluster on Amazon EC2.
...
* Feature guides:
** [[Deploy scripts]] for launching a cluster on a set of machines.
