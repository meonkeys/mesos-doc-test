...
<li> Set up Hadoop's configuration as you would usually do with a new install of Hadoop, following the "instructions on the Hadoop website":http://hadoop.apache.org/common/docs/r0.20.2/index.html (at the very least, you will need to set @JAVA_HOME@ in Hadoop's @conf/hadoop-env.sh@).</li>
...
Note that when you run on a cluster, Hadoop expects to be located on the same path on all nodes (as does Mesos).
