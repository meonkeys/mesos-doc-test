...
There are three unit tests for the Linux Container isolation module. These are disabled by default, but can be enabled using the @--gtest_also_run_disabled_tests@ flag to @alltests@. Note that for these tests to run, you must be running @alltests@ as root (since they actually invoke container commands!). It is probably most convenient to do this in a VM.
